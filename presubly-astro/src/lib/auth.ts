...
function saveSession() {
  ...
  const user = $user.get();
  localStorage.setItem('psb_session', JSON.stringify({ session, user, exp: Date.now() + 3600 * 1000 }));
}
...
export async function loadUserCredits() {
  ...
  if (!user) return;
  try {
    const d = await sbFetch(`/rest/v1/profiles?id=eq.${user.id}&select=credits,is_admin,full_name`, {
...
export async function loadSession() {
  const raw = localStorage.getItem('psb_session');
  if (!raw) return;
  try {
